...
|-------|-------------------|----------------|--------|-------|
| **v0.8.5.2** | `0.8.5.2` / `0.8.5.2` | `0.8.5` | **Current ModTest deploy. Rollback baseline remains v0.8.4.0. Beta 0.8.5.0 artifacts left intact.** | **QoL — hangar-free index load.** Audit found save-load indexing mostly correct, but editor could still kick `BuildIfNeeded` (fallback + wait-path). Fix: `GameLoadBootstrap` sole builder; `EditorSearchHook` wait/UI-hook only; no hangar rebuild; clear stale build lock after scene-host destroy. Keeps v0.8.5.1 SearchStart fix. |
| **v0.8.5.1** | `0.8.5.1` / `0.8.5.1` | `0.8.5` | **Superseded on ModTest by v0.8.5.2. Beta 0.8.5.0 artifacts left intact.** | **FIX — recoverable SearchStart NRE after ApplySuggestion.** Harmony Prefix skip of `SearchRoutine` returned null IEnumerator → stock `SearchStart` → `StartCoroutine(null)` ("routine is null"). Typing after apply crashed until blur/refocus. Fix: do not skip `SearchRoutine`; block void `SearchStart` only while apply-suppressed; clear custom-filter guard on stock `SearchStart` / focus-typing; `RecoverAfterFailedApply` on ApplySuggestion catch. |
...
| ModTest | **v0.8.5.2** (hangar-free index load; rollback baseline v0.8.4.0) | `Kerbal Space Program - ModTest/GameData/KoobalSearchEngine/Plugins/KoobalSearchEngine.dll` |
