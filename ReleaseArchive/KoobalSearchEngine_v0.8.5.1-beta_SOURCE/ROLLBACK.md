...
|-------|-------------------|----------------|--------|-------|
| **v0.8.5.1-beta** | `0.8.5.1` / `0.8.5.1` | `0.8.5` (+BUILD 1) | **Current packaged beta — dual-deployed to main + ModTest.** Rollback baseline remains v0.8.4.0. | Same code/fixes as the v0.8.5.1 lineage row below, packaged as forum beta zip `KoobalSearchEngine_v0.8.5.1-beta.zip` (GameData + Source + LICENSE + Installation Instructions). Annotated tag **`v0.8.5.1-beta`**. Prior code tag **`v0.8.5.1`** kept (points at pre-package commits; beta tag is the release artifact). Author `timbrwolf1121`; `.version` URL/DOWNLOAD → GitHub. |
