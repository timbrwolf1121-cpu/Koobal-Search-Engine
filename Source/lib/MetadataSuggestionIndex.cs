...
namespace PartSearchSuggest
{
    internal sealed class MetadataSuggestionIndex
    {
        ...
        private const int ModViaAuthorPrefixRank = 4;
        private const int ModNameContainsRank = 5;
        ...
        public IEnumerable<PartSuggestion> Match(string query, int maxResults)
        {
            ...
            foreach (AuthorEntry author in _authors)
            {
                ...
                results.Add(new PartSuggestion
                {
                    ...
                    Part = null,
                    IsHistory = false,
                    IndexedPartCount = author.PartCount
                });
            }
            ...
            if (allowBroadMetadata)
            {
                ...
                else if (matchingMods.Count > 1)
                {
                    ...
                    {
                        // Never invent a ModSuite titled from the typed query ("engin"→"Engin").
                        // Only real indexed ModName rows with canonical DisplayName.
