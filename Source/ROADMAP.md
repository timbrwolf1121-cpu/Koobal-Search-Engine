...
## Startup indexing architecture (v0.7.9.1+)

Indexes are **not** built on the main menu. After the user selects **New Game** or **Load Game**, all search indexes build during the subsequent **loading screen** (KSC, flight, or tracking-station scene entry). VAB/SPH entry attaches the search UI only.

| Phase | When | Work |
|-------|------|------|
| **Main menu** | `GameScenes.MAINMENU` | Clear stale indexes only — **no database load** |
| **Save load** | `KSPAddon.Startup.EveryScene` on SPACECENTER / FLIGHT / TRACKSTATION | `GameLoadIndexService`: `ModMetadataCache`, `EditorPartAvailability` (per-save R&D), `SuggestionIndex`, `MetadataSuggestionIndex`, `CategorizerSuggestionIndex` |
| **Editor entry** | `KSPAddon.Startup.EditorAny` | UI hook + dropdown; consume pre-built indexes. **Future:** initial `SubassemblySuggestionIndex` folder scan (see §2) |

**Hook:** `GameLoadBootstrap` (`[KSPAddon(KSPAddon.Startup.EveryScene, false)]`) — `Awake` invalidates on main menu; `Start` starts `GameLoadIndexService.BuildIfNeeded` on first game-bearing scene after save selection.

**Per-save vs global:**

| Data | Scope | Built when |
|------|-------|------------|
| `ModMetadataCache` | Global (GameData `.version` files) | Save-load phase (idempotent singleton) |
| `EditorPartAvailability` | Per-save (`ResearchAndDevelopment.partTechAvailable`, `amountAvailable`) | Save-load phase |
| Part / metadata / categorizer indexes | Per-save (filtered through availability) | Save-load phase |
| Subassembly index (future) | Per-save folder | Editor entry only; incremental on save/delete |

**Log lines (save load):** `Building search index during game load...` → `Search ready (basic)` → `Search ready (full)`.

**Log lines (VAB):** `Search ready (basic)` near-instant if save-load indexing finished; `Search ready (full)` immediately or within seconds if metadata/categorizer still finishing.
