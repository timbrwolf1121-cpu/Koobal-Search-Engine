...
|-------|-------------------|----------------|--------|-------|
| **v0.8.5.3** | `0.8.5.5` / `0.8.5.5` (display `0.8.5.3`) | `0.8.5` (+BUILD 5) | **Current — dual-deployed to main + ModTest.** Rollback baseline remains v0.8.4.0. | **Typing lag fix** + in-place ranking/history/freeze/dropdown fixes + **suggestion Match debounce** + **150+ mod caps** + **Enter relevance rebalance** (whole-token title/name/resource/tag/module; no description/HTML prose) + **main-menu UI scale fix** (no DDOL overlay / no stock scaler copy). Annotated tag **`v0.8.5.3`**. Author `timbrwolf1121`. |
| **v0.8.5.2b** | `0.8.5.4` / `0.8.5.4` (display `0.8.5.2b`) | `0.8.5` (+BUILD 4) | **Superseded by v0.8.5.3.** Rollback baseline remains v0.8.4.0. | **NetKAN dual-host version_edit + packaging.** Same gameplay as v0.8.5.2a; `x_netkan_version_edit` on both hosts; sanitary zip `KoobalSearchEngine_v0.8.5.2b.zip`. Assembly bumped to `0.8.5.4` (letter builds map to next integer). Annotated tag **`v0.8.5.2b`**. Author `timbrwolf1121`. |
| **v0.8.5.2a** | `0.8.5.3` / `0.8.5.3` (display `0.8.5.2a`) | `0.8.5` (+BUILD 3) | **Superseded by v0.8.5.2b.** Rollback baseline remains v0.8.4.0. | **SpaceDock / CKAN Express packaging.** Same gameplay as v0.8.5.2; short player README; internal `KoobalSearchEngine.ckan` → `depends: Harmony2`; sanitary zip `KoobalSearchEngine_v0.8.5.2a.zip`. Assembly bumped to `0.8.5.3` because letter builds cannot live in AssemblyVersion / AVC integers. Annotated tag **`v0.8.5.2a`**. Author `timbrwolf1121`. |
...
As of **v0.8.5.3**, both installs run the same sanitary GameData package
(assembly `0.8.5.5`, player label `0.8.5.3`). The verified-safe rollback baseline remains **v0.8.4.0**.

| Install | Version | Path |
|---------|---------|------|
| Main KSP | **v0.8.5.3** (`0.8.5.5`) | `GameData/KoobalSearchEngine/Plugins/KoobalSearchEngine.dll` |
| ModTest | **v0.8.5.3** (`0.8.5.5`) | `Kerbal Space Program - ModTest/GameData/KoobalSearchEngine/Plugins/KoobalSearchEngine.dll` |

Forum / Desktop / SpaceDock package: `C:\Users\timbr\Desktop\KSE\KoobalSearchEngine_v0.8.5.3.zip`
...
- `v0.8.5.3` — annotated release tag (Enter-only typing halt / lag fix).
- `v0.8.5.2b` — prior (NetKAN version_edit + packaging); kept.
- `v0.8.5.2a` — prior (SpaceDock Express packaging); kept.
